    ...
    public class requests : Controller {
        ...
        public async Task<object> mine () {
            ...
            foreach (var item in requests) {
                ...
                var link =
                    String.Format ("https://tools.wprdc.org/images/pittsburgh/facilities/{0}.jpg",
                        encodedName); // 0
                Request rqs = new Request () {
                    oid = item.IDField,
        ...
        public async Task<string> getMine () {
            ...
            var key = Environment.GetEnvironmentVariable ("CartegraphAPIkey");
            var cartegraphUrl =
                String.Format ("https://cgweb06.cartegraphoms.com/PittsburghPA/api/v1/classes/cgTasksClass?filter=(([RequesterEmail] is equal to \"{0}\"))",
                    user); // 0
            client.DefaultRequestHeaders.Clear ();
            client.DefaultRequestHeaders.Authorization =
                new AuthenticationHeaderValue ("Basic", key);
